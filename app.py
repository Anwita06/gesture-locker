...
def count_fingers(lm, w, h):
    ...
    # Calculate hand scale (wrist to middle finger MCP)
    dx = pts[0][0] - pts[9][0]
    dy = pts[0][1] - pts[9][1]
    hand_scale = np.sqrt(dx*dx + dy*dy)

    # Calculate distance from thumb tip (4) to index MCP (5)
    tx = pts[4][0] - pts[5][0]
    ty = pts[4][1] - pts[5][1]
    thumb_dist = np.sqrt(tx*tx + ty*ty)

    # Thumb is extended (up) if the thumb_dist to hand_scale ratio is greater than 0.38
    if hand_scale > 0:
        ratio = thumb_dist / hand_scale
        if ratio > 0.38:
            up += 1
